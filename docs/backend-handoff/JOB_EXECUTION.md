...
See `KNOWN_LIMITATIONS.md` in this folder for the full list of what's missing as a result, and `../INTEGRATION_NOTES.md` for how the *existing* partial API (`api.py`'s `POST /api/v1/pipeline/run`) works today in its place.
