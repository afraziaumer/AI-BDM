...
- **Checkpoint/cancellation behavior**: `run_pipeline()` accepts optional `progress_cb`/`cancel_check` hooks (see `JOB_EXECUTION.md`), called at plan-start and plan-resolved — but the one LLM call inside `deconstruct_intent()` itself is not interruptible mid-call; cancellation is only checked before/after this stage, not during it.
...
- **Checkpoint/cancellation behavior**: `run_pipeline()`'s `cancel_check()` hook is polled once per discovery round (see `JOB_EXECUTION.md`) — a cancelled run stops starting new rounds and returns what was already committed, never interrupting a round's `asyncio.gather()` batch partway. There is still no checkpoint to *resume* a killed process from beyond the pipeline's own existing per-business commit cache — "cancel and keep what's done" is implemented; "resume exactly where it left off" is not.
