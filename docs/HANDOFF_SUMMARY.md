...
| Backend/Laravel handoff structure | [`docs/backend-handoff/`](backend-handoff/) — the exact folder Section 7 of the Laravel Backend Construction Handoff Guide requires. Contains `PIPELINE_STAGES.md`, `PROVIDERS_AND_LIMITS.md`, `ARTIFACTS_AND_STORAGE.md`, `JOB_EXECUTION.md`, `schemas/*.json`, `KNOWN_LIMITATIONS.md`, `TEST_RESULTS.txt`, and `fixtures/`. The versioned job request/progress/result/error contract (guide Section 4.3–4.6) is implemented (`POST/GET /api/v1/jobs*`) — wraps Stages 1–2 only; see `JOB_EXECUTION.md` for the exact, honestly-stated scope. |
