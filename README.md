...
**Backend/Laravel integration reviewer — start at [`docs/backend-handoff/`](docs/backend-handoff/).** That folder is the required handoff structure per the Laravel Backend Construction Handoff Guide: a per-stage pipeline record (`PIPELINE_STAGES.md`), a consolidated provider/quota table (`PROVIDERS_AND_LIMITS.md`), storage/artifact contracts (`ARTIFACTS_AND_STORAGE.md`), the versioned job request/progress/result/error contract (`JOB_EXECUTION.md` + `schemas/*.json`, Section 4.3–4.6), backend-scoped known limitations (`KNOWN_LIMITATIONS.md`), and the latest test results (`TEST_RESULTS.txt`). The async job API lives at `POST /api/v1/jobs` — see `docs/backend-handoff/JOB_EXECUTION.md` for what it does and doesn't cover (it wraps Stages 1–2 only; Stages 3–8 are a stated, honest gap, not silently ignored).
